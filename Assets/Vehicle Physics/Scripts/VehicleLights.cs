...
public class VehicleLights : MonoBehaviour
{
    ...
    [Tooltip("Type of the light")] public LightType lightType;
    ...
    {
        get { return CommonSettings.indicatorClip; }
    }
    ...
    void Start()
    {
        ...
        if (lightType == LightType.Indicator)
        {
            if (!carController.transform.Find("All Audio Sources/Indicator Sound AudioSource"))
                indicatorSound = CreateAudioSource.NewAudioSource(carController.gameObject,
                    "Indicator Sound AudioSource", 1f, 3f, 1, indicatorClip, false, false, false);
            else
        ...
        switch (lightType)
        {
            ...
            case LightType.BrakeLight:
                Lighting((!carController.lowBeamHeadLightsOn
                    ? (carController._brakeInput >= .1f ? 1f : 0f)
                    : (carController._brakeInput >= .1f ? 1f : .3f)));
                break;
